...
During development, I successfully downloaded **real historical data** from Yahoo Finance for 10 stocks (AAPL, MSFT, TSLA, NVDA, GOOGL, AMZN, META, JPM, KO, WMT) covering 2022-2024, which is stored in `data/stock_database.xlsx` (7,530 rows). However, due to regional network restrictions in my location, the live Yahoo Finance API is not consistently accessible for the demo video. The tool therefore includes a **simulation mode** that generates deterministic synthetic data with the same structure and analysis pipeline, ensuring the product remains fully demonstrable regardless of network conditions. Both modes use the identical analysis engine and produce the same types of visualisations and metrics.
...
- Yahoo Finance API has rate limits and may be inaccessible in certain regions; the tool may need re-launching or using simulation mode
...
I verified all financial formulas independently against standard finance references (Bodie, Kane & Marcus: *Investments*). All data in the database comes from Yahoo Finance, and all analysis results were manually spot-checked for reasonableness. The core analytical logic, design decisions, and interpretation of findings are my own work.
